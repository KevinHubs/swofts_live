...
class HttpServer extends AbstractServer
{
    ...
    protected function registerTcpEvent()
    {
        ...
        if (!isset($swooleListeners[SwooleEvent::TYPE_PORT][0]) || empty($swooleListeners[SwooleEvent::TYPE_PORT][0])) {
            throw new RuntimeException("Please use swoft/tcp-server, run 'composer require swoft/rpc-server'");
        }
        $this->listen = $this->server->listen($this->tcpServerSetting['host'], $this->tcpServerSetting['port'], $this->tcpServerSetting['type']);
        $tcpSetting = $this->getListenTcpSetting();
        $this->listen->set($tcpSetting);
